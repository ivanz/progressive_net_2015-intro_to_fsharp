namespace FsWebApiExample.Controllers
...
open FsWebApiExample
open FsWebApiExample.Rop
open FsWebApiExample.Dtos
...
type CustomersController (fsDao:DataAccessLayer.ICustomerDao) as this =
    // We inject the F# DAO object into the object via the constructor
    ...
    member this.GetExample() : IHttpActionResult = 
        ...
        dto.Extras <- 
            [|
            ExtraDto(Key="A", Value="AValue")
            ExtraDto(Key="B", Value="BValue")
            |]
