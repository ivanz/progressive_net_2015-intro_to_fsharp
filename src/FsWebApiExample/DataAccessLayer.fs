...
type FromDbCustomer = DbCustomer -> RopResult<Customer,DomainMessage>
 
let fromDbCustomerIdiomatic : FromDbCustomer = 
   fun dbCustomer ->
    if dbCustomer = null then 
        fail SqlCustomerIsInvalid
    else
...
let (|KeyNotFound|DuplicateKey|Timeout|AuthenticationError|Other|) (ex:SqlException) =
    match ex.Data0 with 
    | "KNF" -> KeyNotFound
    | "DK" -> DuplicateKey
    | "TO" -> Timeout
