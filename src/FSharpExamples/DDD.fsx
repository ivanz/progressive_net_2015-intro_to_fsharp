...
This session will explore the F# type system, and how it can be used for domain modelling that is 
understandable to the non-technical customer, yet useable as an executable specification.

We’ll see why F# doesn’t allow nulls, and how you can model optional values and choices in a simple and elegant way.

We’ll also demonstrate how to encode business rules by using types, so that you cannot even compile code that doesn’t fit the rules. 
In other words, we will be creating "compile-time" unit tests!
